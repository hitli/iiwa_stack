...
for i = 1:size(robot_data,1)%获得所有旋转矩阵
    jo = quat2rotm(circshift(robot_data(i,4:7),[0,1]));  % circshift(robot_data(i,4:7),[0,1])变换四元数位置，把实部放在第一位
    mb = quat2rotm(circshift(ndi_data(i,4:7),[0,1]));  % tjo*tob*tbm=tjm=tjo1*tob*tbm1 所以要将tmb求逆
    tjo(:,:,i) = [jo,robot_data(i,1:3)';0 0 0 1];
    tmb(:,:,i) = [mb,ndi_data(i,1:3)';0 0 0 1];
end
for i = 1:size(robot_data,1)  %tjo1逆*tjo*tob=tob*tbm1*tbm逆
    for j = 1:size(robot_data,1) %即AX=XB
    count = count + 1;
    B(:,:,count) = tmb(:,:,i)\tmb(:,:,j);  % B=tbm1*tbm逆=tmb1逆*tmb
